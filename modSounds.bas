...
Private Sub CreateBuffers(AmountOfBuffer As Integer, DefaultFile As String)
...
Private Sub SetupDX7Sound(CurrentForm As Form)
...
Private Sub DX7LoadSound(Buffer As Integer, sfile As String)
...
Private Function PlaySoundAnyBuffer(Filename As String, Optional Volume As Byte, Optional PanValue As Byte, Optional LoopIt As Byte) As Integer
...
Private Sub PlaySoundWithPan(Buffer As Integer, Filename As String, Optional Volume As Byte, Optional PanValue As Byte, Optional LoopIt As Byte)
...
Private Sub PanSound(Buffer As Integer, PanValue As Byte)
...
Private Sub VolumeLevel(Buffer As Integer, Volume As Byte)
...
Private Function IsPlaying(Buffer As Integer) As Long
  IsPlaying = SB(Buffer).Buffer.GetStatus
End Function
'Loads what-ever sounds you need
Public Function Load_Sounds()
    SetupDX7Sound frmShoot
    CreateBuffers 4, App.Path + "\snd\cannon.wav"
    DX7LoadSound 0, App.Path + "\snd\cannon.wav"
    DX7LoadSound 1, App.Path + "\snd\explode.wav"
    DX7LoadSound 2, App.Path + "\snd\power-up.wav"
    DX7LoadSound 3, App.Path + "\snd\shield.wav"
    DX7LoadSound 4, App.Path + "\snd\efire.wav"
   
  
End Function
'Plays a sound
Public Function play_sound(ps As Integer)
    Select Case ps
        Case 0
            PlaySoundWithPan 0, App.Path + "\snd\cannon.wav", 100, 50, 0
        Case 1
            PlaySoundWithPan 1, App.Path + "\snd\explode.wav", 100, 50, 0
        Case 2
            PlaySoundWithPan 2, App.Path + "\snd\power-up.wav", 100, 50, 0
        Case 3
            PlaySoundWithPan 3, App.Path + "\snd\shield.wav", 100, 50, 0
        Case 4
            PlaySoundWithPan 4, App.Path + "\snd\efire.wav", 100, 50, 0
